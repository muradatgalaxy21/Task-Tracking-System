...
// Helper function to resize and compress image before uploading
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // 1. Maintain aspect ratio and scale down if exceeding maximum bounds
        if (width > height) {
          if (width > MAX_WIDTH) {
        ...
        if (!ctx) {
          reject(new Error("Failed to get 2d context for image downscaling."));
          return;
        }

        // 2. Draw scaled image on canvas and compress to JPEG format at 0.8 quality
...
export default function ProfileSettingsPanel() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. Compress and optimize image using canvas downscaling
